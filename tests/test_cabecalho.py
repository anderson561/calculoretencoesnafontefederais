...
def test_cabecalho_com_prestador():
    cab = Cabecalho(prestador="Norte Contábil", cnpj="11222333000181", data_emissao="29/07/2026")
    linhas = dict(linhas_cabecalho(cab))
    assert linhas["Prestador"] == "Norte Contábil"
...
def test_cabecalho_vazio_preenche_emissao_com_hoje():
    ...
    # Sem prestador/cnpj: só a emissão aparece.
    assert set(linhas) == {"Emissão"}
    assert linhas["Emissão"] == date.today().strftime("%d/%m/%Y")


def test_cabecalho_omite_prestador_vazio():
    linhas = dict(linhas_cabecalho(Cabecalho()))
    assert "Prestador" not in linhas
