"""Exportação dos relatórios 1 e 2 para um arquivo Excel (.xlsx)."""
...
from .formatacao import linhas_cabecalho, mascarar_documento
...
def _moeda(ws: Worksheet, linha: int, col: int, valor: Decimal, negrito: bool = False):
...
def _bloco_analitico(ws: Worksheet, linha: int, titulo: str, linhas_dados) -> int:
    colunas = ["Documento", "Número NF", "Data", "Nome / Razão Social", "Qtd. Notas",
               "Valor Bruto", "IRRF Retido", "CRF Retido", "INSS Retido"]
    linha = _titulo(ws, linha, titulo, len(colunas))
    linha = _cabecalho(ws, linha, colunas)
    if not linhas_dados:
        ws.cell(row=linha, column=1, value="(sem registros)")
        return linha + 2
    for la in linhas_dados:
        ws.cell(row=linha, column=1, value=mascarar_documento(la.documento, la.tipo))
        ws.cell(row=linha, column=2, value=la.numeros_fmt)
        ws.cell(row=linha, column=3, value=la.datas_fmt)
        ws.cell(row=linha, column=4, value=la.nome or "-")
        ws.cell(row=linha, column=5, value=la.qtd_notas)
        _moeda(ws, linha, 6, la.valor_bruto)
        _moeda(ws, linha, 7, la.total_irrf)
        _moeda(ws, linha, 8, la.total_crf)
        _moeda(ws, linha, 9, la.total_inss)
        linha += 1
    return linha + 2
...
def _aba_analitico(ws: Worksheet, notas: list[NotaCalculada], cab: Cabecalho | None) -> None:
    ...
    linha = _escrever_cabecalho(ws, cab, 9)
    ...
    _autoajustar(ws, [22, 16, 22, 28, 10, 15, 14, 14, 14])


def exportar_excel(
    notas: list[NotaCalculada], caminho: str | Path, cabecalho: Cabecalho | None = None
) -> Path:
    """Gera o arquivo Excel com as duas abas de relatório."""
